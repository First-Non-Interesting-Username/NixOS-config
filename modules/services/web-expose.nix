{ self, ... }:
{
  flake.nixosModules.web-expose =
    ...
    let
      ...
      routerAuth =
        r:
        if r.oidcPlugin != null && r.oidcPlugin.enable then
          null
        else if r.auth != null then
          r.auth
        else if r.public then
          cfg.authelia.defaultPolicy
        else
          null;
      ...
      oidcPluginRouters = lib.filterAttrs (
    ...
    in
    {
      options.custom.web-expose = {
        ...
        authelia = {
          ...
          oidc = {
            enable = lib.mkEnableOption "Authelia as an OIDC Provider" // {
        ...
        lldap = {
          enable = lib.mkEnableOption "LLDAP directory server" // {
          ...
          bootstrap = {
            enable = lib.mkEnableOption "automatic LLDAP user and group bootstrap" // {
            ...
            lib.types.submodule (
              { name, config, ... }:
              {
                options = {
                  ...
                  healthCheck = {
                    enable = lib.mkEnableOption "backend health check" // {
                      default = false;
                    };
                    path = lib.mkOption {
                      type = lib.types.str;
                      ...
                      lib.types.submodule {
                        options = {
                          ...
                          clientId = lib.mkOption {
                            type = lib.types.str;
                            default = if config.oidc != null then config.oidc.client_id else name;
              ...
              dynamicConfigOptions.http = {
                ...
                routers =
                  (lib.mapAttrs (name: r: {
                    ...
                    middlewares =
                      let
                        base = if r.public then [ "public" ] else [ "lan-only" ];
                        authMw =
                          if r.oidcPlugin != null && r.oidcPlugin.enable then
                            [ "${name}-oidc-plugin" ]
                          else
                            lib.optional (r.auth != null) "authelia";
                      in
                      base ++ authMw;
                  }) effectiveRouters)
